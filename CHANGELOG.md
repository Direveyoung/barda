...
### Added
- **테스트 계정 로그인** (`auth/login/page.tsx`): 로그인 페이지에서 test@barda.dev 원클릭 로그인 (프리미엄 포함)

### Changed
- **TODO.md 전면 정리**: 완료 항목 22개로 업데이트, 구조 간결화
- **CLAUDE.md 현재 상태**: v1.5.0 기준으로 업데이트
- **docs/features.md**: v1.3.0~v1.5.0 기능 반영
...
### Added
- **DesktopShell 컴포넌트**: PC에서 모바일 폰 목업 프레임 + QR 코드 사이드바 표시
  - 768px 이상에서 활성화: 좌측 브랜딩/QR/앱스토어 버튼 + 우측 iPhone 목업 프레임
  - QR 코드 SVG 자동 생성 (URL 기반 deterministic 패턴)
  - App Store / Google Play 다운로드 버튼 (플레이스홀더)
  - 폰 목업: 노치, 홈 인디케이터, 둥근 모서리, 그림자 효과
- **랜딩 페이지 전면 리디자인** (화해/파우더룸 스타일)
  - 히어로 배너 캐러셀 (3개 슬라이드, 4초 자동 회전)
  - 카테고리 빠른 메뉴 (4×2 아이콘 그리드)
  - 횡스크롤 기능 카드 (성분 충돌/AM·PM 순서/7일 캘린더)
  - 리스트형 스킨케어 도구 (AI/NEW 배지 포함)
  - 커뮤니티 미리보기 섹션
  - 랭킹 미리보기 카드 (금/은/동 메달)
  - 다크 CTA 배너

### Changed
- `layout.tsx`: DesktopShell 래핑 추가
- `globals.css`: 폰 목업 + 데스크톱 쉘 CSS 추가, `.phone-screen` 내 `fixed` → `sticky` 오버라이드
